...
interface VturbTableProps {
  accounts: VturbAccountAPI[];
  isLoading: boolean;
  onDelete: (account: VturbAccountAPI) => void;
}
...
export function VturbTable({ accounts, isLoading, onDelete }: VturbTableProps) {
      ...
      <Card className="border-border/40 border-dashed">
        <CardContent className="flex flex-col items-center justify-center py-16 text-center gap-2">
          ...
          <p className="text-xs text-muted-foreground/70">
            Acesse Configurações → API do Analytics no VTurb para gerar sua API Key.
          </p>
        </CardContent>
      </Card>
    );
